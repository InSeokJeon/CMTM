...
class Trainer(object):
    ...
    def cycle_dataset(self, mode):
        if mode == 'train':
            for i, vos_data in enumerate(self.train_loader):
                ...
                # model run
                vos_out = self.model(imgs, flows, index=i, epoch=self.epoch)
            ...
            print('[ep{:04d}] Total loss: {:.5f}, iou: {:.5f}'.format(self.epoch, self.stats['loss'].avg, self.stats['iou'].avg))
