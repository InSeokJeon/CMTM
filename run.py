...
def train_ytvos(model):
    train_set = TrainYTVOS('../../DB/VOS/YTVOS18', 'train', clip_n=512)
...
def train_dutsv2_davis(model):
    dutsv2_set = TrainDUTSv2('../../DB/VOS/DUTSv2', clip_n=384)
...
if __name__ == '__main__':
    ...
    if options.train:
        ...
        # Load Pretrained Weight for ytvos Dataset
        pretrained_weight_path = './weights/ytvos_best.pth'
        ...
        for name, params in model.module.named_parameters():
            ...
            elif params.shape != pretrained_state_dict[name].shape:
                print('Size mismatch {} : {} -> {}'.format (name, params.shape, pretrained_state_dict[name].shape))
            else:
                print('Matched {} : {}'.format(name, params.shape))
                
        train_dutsv2_davis(model)
        
    # testing stage
    if options.test:
        model.load_state_dict(torch.load('weights/ytvos_dutsv2_davis_best.pth', map_location='cpu'))
        with torch.no_grad():
            test_davis(model)
            test_fbms(model)
            test_ytobj(model)
